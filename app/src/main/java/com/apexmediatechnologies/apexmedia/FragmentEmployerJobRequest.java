...
public class
FragmentEmployerJobRequest extends Fragment
{
	...
	private EmployerJobRequestAdapter proposalsAdapter;
	private Utility utility;
	private String strGetJobRequest = Application_Constants.Main_URL+"keyword=job_requests";
	String jobId="",siteUserID="";
	private Button btn_find_talent;

	public FragmentEmployerJobRequest()
	{
	}
			...
			Bundle savedInstanceState)
	{
		...
		view=inflater.inflate(R.layout.fragment_employer_job_request,container, false);
		...
		btn_find_talent.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {

				Intent intent = new Intent(getActivity(),FindTalentContractorActivity.class);
		...
		if(utility.checkInternet())
		{
			...
			serviceHandler.registerUser(params, strGetJobRequest, new ServiceHandler.VolleyCallback() {
				...
				public void onSuccess(String result) {
					...
					String requsestId, job_id, job_name, contractor_info, contractor_pic, message, posted_date;
					try {
						if (str_json != null) {
							...
							if (str_status.equalsIgnoreCase("true")) {
								...
								for (int i = 0; i < jArray.length(); i++)
								{
									...
									contractor_info = Obj.getString("contractor_info");
									contractor_pic = Obj.getString("contractor_pic");
									message = Obj.getString("message");
									posted_date = Obj.getString("posted_date");


									JobRequest jobProposals = new JobRequest(requsestId,job_id,job_name,contractor_info,contractor_pic,message,posted_date);
								...
								if (str_status.equals("true")) {
									...
									proposalsAdapter = new EmployerJobRequestAdapter(getActivity(), feedsArrayList);
